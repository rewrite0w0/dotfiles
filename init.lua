...
require('lazy').setup({  -- lazy.nvim의 플러그인 목록과 설정을 정의
	...
	{ 'prettier/vim-prettier', ft = {'javascript', 'typescript', 'css', 'json', 'markdown', 'vue', 'svelte', 'yaml', 'html', 'javascriptreact', 'typescriptreact'}, commit = 'latest' },
...
-- vim.keymap.set('t', '<C-c>', '<C-\\><C-c>', { noremap = true, silent = true })

vim.g['prettier#exec_cmd_path'] = '경로/prettier.cmd' -- 경로 지정, 하드코딩이므로 나중에 변경할 것
-- 자동 명령: 특정 조건에서 자동 실행
vim.api.nvim_create_autocmd('BufWritePre', {  -- 파일 저장 전 자동 명령
    pattern = { '*.js', '*.jsx', '*.json', '*.css', '*.md', '*.ts', '*.tsx', '*.html' },  -- 지정된 파일 형식에 적용
